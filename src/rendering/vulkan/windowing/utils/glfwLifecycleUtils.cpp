...
#include "glfwLifecycleUtils.h"

#include "../../vulkanPCH.h"

void glfwLifecycleUtils::createInstance() {
...
VkSurfaceKHR glfwLifecycleUtils::createSurface(const VkInstance instance,
                                               GLFWwindow *window) {
  ...
  if (glfwCreateWindowSurface(instance, window, nullptr, &surface) !=
    ...
    throw std::runtime_error(
      fmt::format("Failed to create window surface: {}", err));
  }

  return surface;
}

GLFWwindow *glfwLifecycleUtils::createWindow(const int width, const int height,
                                             const char *title) {
  ...
  if (window == nullptr) {
    ...
    glfwGetError(&err);
    throw std::runtime_error(
      fmt::format("Failed to create GLFW window: {}", err));
  }
